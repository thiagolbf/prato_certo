...
- **Decisão**: adotamos um **monolito modular**. Um único processo FastAPI, um único artefato de deploy, organizado internamente em módulos por capacidade de negócio, cada um com a mesma estrutura interna: `router` (HTTP) → `service` (regra) → `repository` (persistência). Módulos conversam por chamada de função entre serviços, nunca por acesso direto ao repositório alheio. No código, **cada módulo é um pacote próprio** que reúne suas entidades, schemas, repositório, service e router (*package by feature*); não existem pastas globais por camada (`models/`, `services/`, `routers/`), porque elas espalhariam cada módulo por várias pastas e esconderiam a fronteira acima. O que é transversal (configuração, sessão de banco, dia operacional, exceções) fica num pacote `core`, que não conhece nenhum módulo.
...
- **Decisão**: aplicamos orientação a objetos pelo critério **"existe invariante a proteger ou dependência a trocar?"**. Onde sim, classe com comportamento; onde não, função.
  1. **Entidades com comportamento** nos módulos `identidade`, `catalogo` e `vendas`. A regra vive em métodos da entidade; o service orquestra — carrega, chama o método, persiste, confirma a transação. Concretamente:
     ...
     - `CardapioData.herdar_de(anterior, data)` concentra a herança automática da RN-08, já descartando itens desativados (RN-12). Devolve um cardápio resolvido em memória, que nunca é adicionado à sessão: o herdado não é gravado, e só a confirmação ou substituição pelo ADMIN persiste o cardápio da data (RN-08, RN-10).
  ...
  5. **Services como classes** com dependências recebidas no construtor, o que permite testá-los com repositório falso, sem banco. A injeção de dependência é a do próprio FastAPI, **sem container de DI**: funções de dependência (`Depends`) montam, a cada requisição, a cadeia sessão → usuário autenticado → repositório → service, e só routers e essas funções conhecem o FastAPI. Service e repositório recebem tudo pelo construtor e não importam nada do framework. Nos testes, a troca é por `app.dependency_overrides` ou instanciando o service diretamente.
...
Os dois atores estão separados porque têm perfis de uso opostos: o operador usa uma única tela, sob pressão, dezenas de vezes por dia; o ADMIN usa várias telas, sem pressa, poucas vezes por semana — e, quando trabalha no balcão, usa a mesma tela do operador, com a mesma exigência de velocidade. Essa assimetria justifica tanto a decisão de manter o app no cliente (ADR-002) quanto a expiração por inatividade mais curta da sessão do ADMIN (ADR-006).
